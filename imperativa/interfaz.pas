...
Var
   ...
   lista:t_lista;
BEGIN
  abrir(lista);  // abre el archivo en caso de que este en mem secundaria, sino inicializa el vector.
  respuesta:=1;
  ...
  cerrar(lista); // cierra el archivo en caso de que este en mem secundaria, sino no hace nada.
END;
...
control:= 1;
   ...
   begin
     ...
     elemento.alta:=true;
     insertar(lista,elemento); // Se carga el elemento ingresado en la lista.
...
Begin
    ...
    clrscr;
      ...
      elemento := devolver_elemento_alta_true(lista,i);
      if elemento.tipo_evento <> '' then
      begin
...
begin
  ...
  readln (buscado);
  buscar_por_tipo(lista,buscado,elemento,control,pos); // pos recibe la posicion del primero de los elementos que coincide.
  if control = false then
        begin
     ...
     begin
       repeat     // Repite hasta que no encuentre mas elementos del tipo del evento (para mostrar todos)
            ...
            Write('Presione una tecla para mostrar el siguiente elemento ');
            readkey;
            coincideSiguiente(lista,buscado,elemento,control,pos); // DEMASIADOS PARAMETROS (pero funciona)
       until control = false;
       end;
end;
